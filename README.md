...
Скрипт створює 4 тестових користувачів (по одному на роль), 6 відділів і
банк із ~53 метрик (розд. 3.1 ТЗ). Пароль усіх тестових акаунтів — `password`.
...
### Phase 1 (MVP) - функціонал завершено
- [x] Базова архітектура (Next.js + PostgreSQL + Prisma)
- [x] Авторизація (JWT) + захист роутів (middleware) + рольовий доступ
- [x] Банк метрик (CRUD + архівування, F-01..05)
- [x] Розрахунок KPI і бонусів
- [x] CRM для Operations (конструктор конфігурацій, F-06..14)
- [x] Флоу погодження з тімлідом (F-15..17)
- [x] FRONT / DATA / HISTORY з авторозрахунком (F-18..24)
- [x] Дашборд менеджера (F-25..27)
- [x] Дашборд керівництва (F-28..30)
